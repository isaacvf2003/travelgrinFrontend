...
const navItems: Array<{ label: string; icon: ComponentType<{ className?: string }>; section: AdminSection }> = [
  ...
  { label: "Como funciona", icon: FileText, section: "como-funciona" },
];
    ...
    <div className="flex min-h-screen overflow-x-hidden bg-slate-50">
      ...
      >
        <div className="flex items-center gap-3 border-b border-slate-100 px-5 py-5">
          ...
          <div>
            <p className="text-sm font-bold leading-tight text-slate-800">TravelGrinAdmin</p>
        ...
        <nav className="py-4 px-3 space-y-0.5">
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.section}
                href={item.section === "panel" ? "/admin" : `/admin?section=${item.section}`}
                onClick={() => setSidebarOpen(false)}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all group
                  ${isActive ? "bg-indigo-50 text-indigo-700" : "text-slate-500 hover:bg-slate-50 hover:text-slate-800"}`}
              >
                <Icon
                  className={`w-4 h-4 flex-shrink-0 ${isActive ? "text-indigo-600" : "text-slate-400 group-hover:text-slate-600"}`}
                />
                <span className="flex-1 text-left">{item.label}</span>
                {isActive ? <ChevronRight className="w-3 h-3 text-indigo-400" /> : null}
              </Link>
            );
          })}
        </nav>
      ...
      <div className="flex min-h-screen min-w-0 flex-1 flex-col lg:ml-[220px]">
        ...
        <main className="admin-page-content flex-1 min-w-0 overflow-x-hidden p-3 sm:p-4 lg:p-7">{children}</main>
      </div>
    </div>
  );
}
