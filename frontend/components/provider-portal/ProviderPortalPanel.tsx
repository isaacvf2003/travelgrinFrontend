...
export default function ProviderPortalPanel() {
  ...
  const mfaCopy = useMemo(() => ({
    mfaSetupTitle:
      locale === "en" ? "Set up Multi-Factor Authentication (MFA)" :
      locale === "pt" ? "Configurar Autenticação de Dois Fatores (MFA)" :
      locale === "it" ? "Configura l'autenticazione a due fattori (MFA)" :
      "Configurar Doble Factor (MFA)",
    mfaSetupBody:
      locale === "en" ? "To protect your account, link it with your authenticator app (Google Authenticator, Authy, etc.)." :
      locale === "pt" ? "Para proteger sua conta, vincule-a ao seu aplicativo de autenticação (Google Authenticator, Authy, etc.)." :
      locale === "it" ? "Per proteggere il tuo account, collegalo alla tua app di autenticazione (Google Authenticator, Authy, ecc.)." :
      "Para proteger tu cuenta, vinculala con tu aplicación de autenticación (Google Authenticator, Authy, etc.).",
    mfaSetupSecretLabel:
      locale === "en" ? "Manual Secret Code" :
      locale === "pt" ? "Código Secreto Manual" :
      locale === "it" ? "Codice segreto manuale" :
      "Código Secreto Manual",
    mfaSetupHint:
      locale === "en" ? "Double-click to copy" :
      locale === "pt" ? "Clique duas vezes para copiar" :
      locale === "it" ? "Fai doppio clic per copiare" :
      "Hacé doble clic para copiar",
    mfaCodeLabel:
      locale === "en" ? "Enter the 6-digit code:" :
      locale === "pt" ? "Insira o código de 6 dígitos:" :
      locale === "it" ? "Inserisci il codice a 6 cifre:" :
      "Ingresa el código de 6 dígitos:",
    mfaSetupSubmit:
      locale === "en" ? "Confirm and Activate" :
      locale === "pt" ? "Confirmar e Ativar" :
      locale === "it" ? "Conferma e attiva" :
      "Confirmar y Activar",
    mfaVerifyTitle:
      locale === "en" ? "Security Verification" :
      locale === "pt" ? "Verificação de Segurança" :
      locale === "it" ? "Verifica di sicurezza" :
      "Verificación de Seguridad",
    mfaVerifyBody:
      locale === "en" ? "Enter the 6-digit code generated by your authenticator app for this email." :
      locale === "pt" ? "Insira o código de 6 dígitos gerado pelo seu aplicativo de autenticação para este e-mail." :
      locale === "it" ? "Inserisci il codice a 6 cifre generato dalla tua app di autenticazione per questa email." :
      "Ingresa el código de 6 dígitos generado por tu aplicación de autenticación para este email.",
    mfaVerifySubmit:
      locale === "en" ? "Log In" :
      locale === "pt" ? "Iniciar Sessão" :
      locale === "it" ? "Accedi" :
      "Iniciar Sesión",
    mfaVerifying:
      locale === "en" ? "Verifying..." :
      locale === "pt" ? "Verificando..." :
      locale === "it" ? "Verifica in corso..." :
      "Verificando...",
    mfaVerifyCodeLabel:
      locale === "en" ? "Authenticator Code:" :
      locale === "pt" ? "Código do Autenticador:" :
      locale === "it" ? "Codice di autenticazione:" :
      "Código de Autenticador:",
    mfaInstallTitle:
      locale === "en" ? "Don't have an authenticator app?" :
      locale === "pt" ? "Não tem un aplicativo de autenticação?" :
      locale === "it" ? "Non hai un'app di autenticazione?" :
      "¿No tenés un autenticador?",
    mfaInstallBody:
      locale === "en" ? "Download Google Authenticator on your phone from the official stores:" :
      locale === "pt" ? "Baixe o Google Authenticator no seu celular nas lojas oficiais:" :
      locale === "it" ? "Scarica Google Authenticator sul tuo telefono dagli store ufficiali:" :
      "Descargá Google Authenticator en tu celular desde las tiendas oficiales:",
    mfaGooglePlay:
      locale === "en" ? "Download for Android" :
      locale === "pt" ? "Baixar para Android" :
      locale === "it" ? "Scarica per Android" :
      "Descargar para Android",
    mfaAppStore:
      locale === "en" ? "Download for iOS" :
      locale === "pt" ? "Baixar para iOS" :
      locale === "it" ? "Scarica per iOS" :
      "Descargar para iOS",
  }), [locale]);
        ...
        {mfaStep === "setup" ? (
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm max-w-lg mx-auto">
            <h3 className="text-xl font-bold text-slate-900 flex items-center gap-2">
              ...
              {mfaCopy.mfaSetupTitle}
            </h3>
            <p className="mt-2 text-sm text-slate-600 leading-relaxed">
              {mfaCopy.mfaSetupBody}
            </p>
            ...
            <div className="mt-6 flex flex-col items-center gap-4 bg-slate-50 p-5 rounded-2xl border border-slate-100">
              ...
              <div className="text-center w-full">
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider block">{mfaCopy.mfaSetupSecretLabel}</span>
                ...
                <span className="text-[11px] text-slate-400 block mt-1">{mfaCopy.mfaSetupHint}</span>
              </div>
            </div>
            ...
            <div className="mt-6 space-y-4">
              ...
              >
                {mfaSubmitting ? mfaCopy.mfaVerifying : mfaCopy.mfaSetupSubmit}
              </button>

              <div className="border-t border-slate-100 mt-6 pt-5 space-y-3">
                <h4 className="text-sm font-semibold text-slate-800">{mfaCopy.mfaInstallTitle}</h4>
                <p className="text-xs text-slate-500">{mfaCopy.mfaInstallBody}</p>
                <div className="flex flex-wrap gap-2.5 pt-1">
                  <a
                    href="https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 rounded-xl bg-slate-50 border border-slate-200 px-3.5 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-100 transition-colors"
                  >
                    <img src="/google-play.webp" alt="Google Play" className="h-4 w-auto object-contain" />
                    <span>{mfaCopy.mfaGooglePlay}</span>
                  </a>
                  <a
                    ...
                    className="inline-flex items-center gap-2 rounded-xl bg-slate-50 border border-slate-200 px-3.5 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-100 transition-colors"
                  >
                    <img src="/logotipo-de-apple.webp" alt="App Store" className="h-4 w-auto object-contain" />
                    <span>{mfaCopy.mfaAppStore}</span>
                  </a>
                </div>
              </div>
            </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm max-w-md mx-auto">
            <h3 className="text-xl font-bold text-slate-900 flex items-center gap-2">
              <ShieldCheck className="h-6 w-6 text-[#0B8FA3]" />
              {mfaCopy.mfaVerifyTitle}
            </h3>
            <p className="mt-2 text-sm text-slate-600">
              {mfaCopy.mfaVerifyBody}
            </p>

            <div className="mt-6 space-y-4">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">
                  {mfaCopy.mfaVerifyCodeLabel}
                </label>
                <input
                  type="text"
                  maxLength={6}
              ...
              <button
                ...
                className="w-full h-12 rounded-2xl bg-[#0B8FA3] text-sm font-semibold text-white hover:opacity-95 disabled:opacity-60 flex items-center justify-center transition-opacity"
              >
                {mfaSubmitting ? mfaCopy.mfaVerifying : mfaCopy.mfaVerifySubmit}
              </button>
